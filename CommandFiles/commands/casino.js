...
const gamesInfo = {
  ...
  Roulette: "**🎡 Roulette** - Bet on where the ball will land on the spinning wheel.",
  Poker: "**🃏 Poker** - Build the best hand and outplay your opponents.",
  Craps: "**🎲 Craps** - Roll the dice and try your luck with this fast-paced game.",
  Baccarat: "**👑 Baccarat** - Bet on the player, banker, or a tie in this classic card game.",
  Slots: "**🎰 Slots** - Spin the reels and match symbols to win big.",
  Sibco: "**🎯 Sibco** - A strategic dice game, where high stakes meet high rewards.",
};
...
export async function entry({ input, output, money, icon, cancelCooldown = function() {}, Inventory, cooldown, CassExpress }) {
  /*if (input.isAdmin) {
    cancelCooldown();
  }*/
  let [inf, bet, ...additionalArgs] = input.arguments;
  const game = Object.keys(gamesInfo).find(i => String(i).toLowerCase() === String(inf).toLowerCase());
  ...
  if (!game || !gamesInfo[game] || cooldown) {
    cancelCooldown();
    return output.reply(
      `🎲 **Casino Menu** 🎰\n\n${!cooldown ? `Available Games:` : `⏰ Cooling down, please wait for **${cooldown}**s!` }\n\n` +
      Object.values(gamesInfo).join("\n\n") +
      `\n\n[font=typewriter]Use the command: {prefix}casino {gamename} {betamount}[:font=typewriter]\n\n$**${pCy(playerMoney)}** **${inventory.size()}/8**`,
    );
  }

  const gamesWithBets = ['Blackjack', 'Roulette', 'Poker', 'Craps', 'Baccarat', 'Slots', 'Sibco'];
  if (gamesWithBets.includes(game)) {
    ...
    if (!inventory.has("highRollPass") && bet > 100000) {
      cancelCooldown();
      return output.reply(`You need a 🃏 **HighRoll Pass** to place bets over 100000`);
    }
  }
  ...
  const outcomes = {
    Blackjack() {
      ...
      const win = playerTotal > dealerTotal && playerTotal <= 21 || dealerTotal > 21;
      netGain = win ? bet : -bet;
      resultMessage = `**🎴 Blackjack Result**\n` +
        `Player's Total: ${playerTotal} ${playerTotal > 21 ? "💥 (Busted)" : ""}\n` +
        `Dealer's Total: ${dealerTotal} ${dealerTotal > 21 ? "💥 (Busted)" : ""}\n` +
    ...
    Roulette() {
      ...
      const win = (playerBet === 'number' && winningNumber === parseInt(additionalArgs[1])) ||
                  (playerBet === 'red' && winningNumber % 2 !== 0) ||
                  (playerBet === 'black' && winningNumber % 2 === 0) || (!playerBet && winningNumber % 2 !== 0);
      const ballPosition = `🎱 ${winningNumber}`;
      netGain = win ? playerBet === 'number' ? bet * 25 : Math.round(bet / 2.2) : -bet;
      resultMessage = `**🎡 Roulette Result**\n` +
        `Ball landed on: ${ballPosition} ${playerBet === 'black' ? `(You win if the number is even)` : playerBet === 'red' ? `(You win if the number is not even)` : ""}\n` +
        `${win ? "🎉 You won!" : "❌ You lost!"}\n` +
  ...
  if (gamesWithBets.includes(game)) {
    ...
    resultMessage += `\n\n$**${pCy(playerMoney)}** **${inventory.size()}/8** (**${netGain >= 0 ? `+${pCy(netGain)}` : pCy(netGain) }**)`;
