...
export const meta = {
  ...
  author: "Liane Cagara",
  waitingTime: 5,
  description: "A Bank Command that looks similar to Marin's",
  category: "banking",
  noPrefix: "both",
};
...
export async function entry({
  ...
  const neededCrops = 2000;
  const userCrops = totalReducer(totalCrops);

  if (true) {
    return message.reply(
      `🏛️ The **Bank** is currently shutting down due to being unable to comply with **Cassidy's** new rules and regulations. This is your last chance to **withdraw.**\n\nMore Options:\n⦿ Bank Withdraw 🚫\n⦿ Bank Balance 🚫`,
    );
  }
  if (
    userCrops < neededCrops &&
    String(args[0]).toLowerCase() !== "balance" &&
    String(args[0]).toLowerCase() !== "withdraw"
  ) {
    return message.reply(
      `🌾 You need to **harvest** **${neededCrops - userCrops} more crops** to unlock this command.\nHarvested Crops: ${userCrops}/${neededCrops} (${Math.floor(userCrops / neededCrops) * 100}%)\n\nMore Options:\n⦿ Bank Withdraw\n⦿ Bank Balance`,
    );
  }
  ...
  if (command === "richest") {
    ...
    const messageText = `🤴🏻👑 Richest users in the bank bot: \n\n${(
      await Promise.all(
  ...
  if (command === "deposit") {
    ...
    if (userDice > cassidyBotDice) {
      outcomeMessage += `Congratulations! You won $10 with a result of ${userDice}.`;
      bankData[user].bank += 10;
    } else if (userDice < cassidyBotDice) {
      outcomeMessage += `Cassidy bot won $10 with a result of ${cassidyBotDice}.`;
      bankData[user].bank -= 10;
    } else {
    ...
    if (bankBal < betAmount) {
      return message.reply(
        `${lianeBank}\n\n✧ You don't have enough bank balance for this bet. Try to deposit a  bank balance.`,
      );
    }
    if (betAmount > 100000 && !inventory.has("highRollPass")) {
      return message.reply(
